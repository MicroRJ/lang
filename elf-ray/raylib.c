...
Vector2 tovec2(elf_ThreadState *R, elf_Table *tab) {
	return CLITERAL(Vector2) {
		elf_tabgetnum(tab,R->cache.x),
		elf_tabgetnum(tab,R->cache.y),
	};
}


Vector3 tovec3(elf_ThreadState *R, elf_Table *tab) {
	return CLITERAL(Vector3) {
		elf_tabgetnum(tab,R->cache.x),
		elf_tabgetnum(tab,R->cache.y),
		elf_tabgetnum(tab,R->cache.z),
	};
}


Rectangle torec(elf_ThreadState *R, elf_Table *tab) {
	return CLITERAL(Rectangle) {
		elf_tabgetnum(tab,R->cache.x),
		elf_tabgetnum(tab,R->cache.y),
		elf_tabgetnum(tab,R->cache.width),
		elf_tabgetnum(tab,R->cache.height),
	};
}


elf_libfundecl int raylib_GuiEnable(elf_ThreadState *R) {
	GuiEnable();
	return 0;
}


elf_libfundecl int raylib_GuiDisable(elf_ThreadState *R) {
	GuiDisable();
	return 0;
}


elf_libfundecl int raylib_GetTextWidth(elf_ThreadState *R) {
	char *text = elf_getstr(R,0)->c;
	elf_locint(R,GetTextWidth(text));
	return 1;
}


elf_libfundecl int raylib_GuiDrawText(elf_ThreadState *R) {
	elf_ensure(R->call->nx == 5);
	char *text = elf_getstr(R,0)->c;
	int size = elf_getint(R,1);
  	GuiSetStyle(DEFAULT,TEXT_SIZE,size);
	Rectangle bounds = torec(R,elf_gettab(R,2));
	int alignment = elf_getint(R,3);
	elf_int color = elf_getint(R,4);
	GuiDrawText(text,bounds,alignment,UNPACK(color));
	return 0;
}


elf_libfundecl int raylib_GuiSlider(elf_ThreadState *R) {
	elf_ensure(R->call->nx == 9);
	Rectangle bounds = CLITERAL(Rectangle) {
		elf_getnum(R,0),elf_getnum(R,1),
		elf_getnum(R,2),elf_getnum(R,3),
	};
	char *textLeft = elf_getstr(R,4)->c;
	char *textRight = elf_getstr(R,5)->c;
	float value = elf_getnum(R,6);
	float minValue = elf_getnum(R,7);
	float maxValue = elf_getnum(R,8);
	elf_locint(R,GuiSlider(bounds,textLeft,textRight,&value,minValue,maxValue));
	return 1;
}


elf_libfundecl int raylib_GuiProgressBar(elf_ThreadState *R) {
	elf_ensure(R->call->nx == 9);
	Rectangle bounds = CLITERAL(Rectangle) {
		elf_getnum(R,0),elf_getnum(R,1),
		elf_getnum(R,2),elf_getnum(R,3),
	...
	float minValue = elf_getnum(R,7);
	float maxValue = elf_getnum(R,8);
	elf_locint(R,GuiProgressBar(bounds,textLeft,textRight,&value,minValue,maxValue));
	return 1;
}


elf_libfundecl int raylib_GetRayCollisionSphere(elf_ThreadState *R) {
	elf_ensure(R->call->nx == 4);
	Ray ray = CLITERAL(Ray) {tovec3(R,elf_gettab(R,0)),tovec3(R,elf_gettab(R,1))};
	Vector3 center = tovec3(R,elf_gettab(R,2));
	float radius = elf_getnum(R,3);

	RayCollision collision = GetRayCollisionSphere(ray,center,radius);
	elf_Table *point = elf_newloctab(R);
	elf_tabsetnumfld(point,R->cache.x,collision.point.x);
	elf_tabsetnumfld(point,R->cache.y,collision.point.y);
	elf_tabsetnumfld(point,R->cache.w,collision.point.z);
	elf_Table *normal = elf_newloctab(R);
	elf_tabsetnumfld(normal,R->cache.x,collision.normal.x);
	elf_tabsetnumfld(normal,R->cache.y,collision.normal.y);
	elf_tabsetnumfld(normal,R->cache.w,collision.normal.z);
	elf_Table *tab = elf_newloctab(R);
	elf_tabsetintfld(tab,elf_newlocstr(R,"hit"),collision.hit);
	elf_tabsetnumfld(tab,elf_newlocstr(R,"distance"),collision.distance);
	elf_tabsettabfld(tab,elf_newlocstr(R,"point"),point);
	elf_tabsettabfld(tab,elf_newlocstr(R,"normal"),normal);
	elf_loctab(R,tab);
	return 1;
}


elf_libfundecl int raylib_RayCollisionSphereHit(elf_ThreadState *R) {
	elf_ensure(R->call->nx == 4);
	Ray ray = CLITERAL(Ray) {tovec3(R,elf_gettab(R,0)),tovec3(R,elf_gettab(R,1))};
	Vector3 center = tovec3(R,elf_gettab(R,2));
	float radius = elf_getnum(R,3);
	RayCollision collision = GetRayCollisionSphere(ray,center,radius);
	elf_locint(R,collision.hit);
	return 1;
}
...
elf_libfundecl int raylib_GetTime(elf_ThreadState *R) {
	elf_locnum(R,GetTime());
	return 1;
}
...
elf_libfundecl int raylib_GetScreenWidth(elf_ThreadState *R) {
	elf_locint(R,GetScreenWidth());
	return 1;
}


elf_libfundecl int raylib_GetScreenHeight(elf_ThreadState *R) {
	elf_locint(R,GetScreenHeight());
	return 1;
}
...
elf_Table *raylib_putvec2(elf_ThreadState *R, Vector2 vec) {
	elf_Table *tab = elf_newloctab(R);
	elf_tabsetnumfld(tab,R->cache.x,vec.x);
	elf_tabsetnumfld(tab,R->cache.y,vec.y);
	elf_loctab(R,tab);
	return tab;
}


Camera2D tocam2d(elf_ThreadState *R, elf_Table *tab) {
	Camera2D result = {0};
	result.offset = tovec2(R,elf_tabgettab(tab,elf_newlocstr(R,"offset")));
	result.target = tovec2(R,elf_tabgettab(tab,elf_newlocstr(R,"target")));
...
elf_libfundecl int raylib_BeginMode2D(elf_ThreadState *R) {
	...
	BeginMode2D(tocam2d(R,elf_gettab(R,0)));
	return 0;
}
...
elf_libfundecl int raylib_GetWorldToScreen2D(elf_ThreadState *R) {
	...
	raylib_putvec2(R,GetWorldToScreen2D(tovec2(R,elf_gettab(R,1)),tocam2d(R,elf_gettab(R,0))));
	return 1;
}
...
elf_libfundecl int raylib_GetScreenToWorld2D(elf_ThreadState *R) {
	elf_ensure(R->call->nx == 2);
	raylib_putvec2(R,GetScreenToWorld2D(tovec2(R,elf_gettab(R,1)),tocam2d(R,elf_gettab(R,0))));
	return 1;
}
...
elf_libfundecl int raylib_SetShapesTexture(elf_ThreadState *R) {
	...
	Rectangle src = CLITERAL(Rectangle){
		elf_getnum(R,1),elf_getnum(R,2),elf_getnum(R,3),elf_getnum(R,4)
	};
...
elf_libfundecl int raylib_DrawCircle(elf_ThreadState *R) {
	elf_ensure(R->call->nx == 4);
	elf_int color = elf_getint(R,3);
	DrawCircle(elf_getint(R,0),elf_getint(R,1),elf_getint(R,2),UNPACK(color));
	return 0;
}


elf_libfundecl int raylib_DrawCircleLines(elf_ThreadState *R) {
	elf_ensure(R->call->nx == 4);
	elf_int color = elf_getint(R,3);
	DrawCircleLines(elf_getint(R,0),elf_getint(R,1),elf_getint(R,2),UNPACK(color));
	return 0;
}


elf_libfundecl int raylib_DrawLine(elf_ThreadState *R) {
	elf_ensure(R->call->nx == 5);
	elf_int color = elf_getint(R,4);
	DrawLine(
	elf_getnum(R,0),elf_getnum(R,1),
...
elf_libfundecl int raylib_load(elf_ThreadState *R) {
	...
	elf_registerint(R,"elf.ray.TEXT_ALIGN_TOP",TEXT_ALIGN_TOP);
	elf_registerint(R,"elf.ray.TEXT_ALIGN_MIDDLE",TEXT_ALIGN_MIDDLE);
	elf_registerint(R,"elf.ray.TEXT_ALIGN_BOTTOM",TEXT_ALIGN_BOTTOM);

	elf_registerint(R,"elf.ray.TEXT_ALIGN_LEFT",TEXT_ALIGN_LEFT);
	elf_registerint(R,"elf.ray.TEXT_ALIGN_CENTER",TEXT_ALIGN_CENTER);
	elf_registerint(R,"elf.ray.TEXT_ALIGN_RIGHT",TEXT_ALIGN_RIGHT);


	elf_register(R,"elf.ray.GuiEnable",raylib_GuiEnable);
	elf_register(R,"elf.ray.GuiDisable",raylib_GuiDisable);
	elf_register(R,"elf.ray.GuiSlider",raylib_GuiSlider);
	elf_register(R,"elf.ray.GuiProgressBar",raylib_GuiProgressBar);
	elf_register(R,"elf.ray.GuiDrawText",raylib_GuiDrawText);

	elf_register(R,"elf.ray.GetRayCollisionSphere",raylib_GetRayCollisionSphere);
	elf_register(R,"elf.ray.RayCollisionSphereHit",raylib_RayCollisionSphereHit);
	...
	elf_register(R,"elf.ray.GetTime",raylib_GetTime);
	...
	elf_register(R,"elf.ray.GetScreenWidth",raylib_GetScreenWidth);
	elf_register(R,"elf.ray.GetScreenHeight",raylib_GetScreenHeight);
	...
	elf_register(R,"elf.ray.DrawLine",raylib_DrawLine);
	elf_register(R,"elf.ray.DrawCircle",raylib_DrawCircle);
	elf_register(R,"elf.ray.DrawCircleLines",raylib_DrawCircleLines);
